...
input = [(i, tuple([int(j) for j in k.split(',')])) for i,k in input]

def arrangements( arrangement, nums ):
    if not nums and all([i!='#' for i in arrangement]):
        return [arrangement]
    if not nums:
        return []
    r = []
    ...
    for i, c in enumerate(arrangement):
        if i+cur > len(arrangement):
            continue
        if i-1 >= 0 and arrangement[i-1] == '#':
            continue
        if i+cur < len(arrangement) and arrangement[i+cur] == '#':
            continue
        if any([j=='.' for j in arrangement[i:i+cur]]):
            continue
        narrangement = ''.join(['.' if j <= i+cur else k for j,k in enumerate(arrangement)])
        for j in arrangements( narrangement, nums[1:] ):
            r.append((j[:i] + '#'*cur + j[i+cur:]).replace('?', '.'))
        if arrangement[i] == '#':
            break
    return r

s = 0
for ar, nums in input:
    v = arrangements(ar, nums)
    s += len(v)
print(s)

# v = arrangements('?###????????', (3,2,1))
